...
# color aesthetic
ggplot(data = bechdel) +
    geom_point(mapping = aes(x=budget,y=domgross,shape=binary),alpha=0.5, color="red")
...
# shape aesthetic
ggplot( data = bechdel) +
    geom_point(mapping = aes(x =budget , y=intgross , shape=binary ))
```
...
ggplot(bechdel, aes(x = binary ,
                  y = budget)) +
  geom_col(aes(fill = binary), width =0.3 )
```
...
ggplot(data=bechdel,aes(x = clean_test))+
  geom_bar(aes(fill = clean_test))+labs(title="praa",
x="x-axis",y="y-axis",caption="data source=anna") 
```
